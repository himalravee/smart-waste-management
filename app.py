...
def load_class_indices(json_path="class_indices.json"):
    ...
    with open(json_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    return data  # list of class names in correct order
...
try:
    ...
    st.info(f"{len(class_names)} classes loaded from `class_indices.json`")
except Exception as e:
...
st.sidebar.write(pd.DataFrame.from_dict(st.session_state.counts, orient="index", columns=["count"]).sort_values("count", ascending=False))
